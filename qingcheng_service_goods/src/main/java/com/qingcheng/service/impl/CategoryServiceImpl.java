...
public class CategoryServiceImpl implements CategoryService {
    ...
    public List<Map> findCategoryTree() {
        System.out.println("从缓存中拿去");
        List<Map> o = (List<Map>) redisTemplate.boundValueOps(CacheKey.CATEGROY_TREE).get();
        return o;
    }

    public void savefindCategoryTreeRedis() {
        //查询is_show==1
        Example example=new Example(Category.class);
        Example.Criteria criteria = example.createCriteria();
        ...
        List<Map> findCategoryTreeredis = findByParentId(categoryList, 0);
        redisTemplate.boundValueOps(CacheKey.CATEGROY_TREE).set(findCategoryTreeredis);
